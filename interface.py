...
class App:
    ...
    def _criar_frames(self):
        ...
        self.frame_detalhes = tk.Frame(
            self.master, width=535, background=co1, relief="solid", borderwidth=1
        )
    ...
    def _criar_widgets_detalhes(self):
        ...
        self.e_nome = tk.Entry(
            self.frame_detalhes, width=30, justify=tk.LEFT, relief="solid"
        )
        ...
        self.e_telefone = tk.Entry(
            self.frame_detalhes, width=20, justify=tk.LEFT, relief="solid"
        )
        ...
        self.c_sexo = ttk.Combobox(
            self.frame_detalhes, width=8, font=("Ivy 8 bold"), justify="center"
        )
        ...
        self.c_curso = ttk.Combobox(
            self.frame_detalhes, width=20, font=("Ivy 8 bold"), values=cursos
        )
    ...
    def _criar_widgets_botoes(self):
        # Frame de busca
        frame_procurar = tk.Frame(
            self.frame_botoes_e_busca, background=co1, relief="groove", borderwidth=1
        )
        frame_procurar.pack(fill="x", padx=10, pady=10)
        tk.Label(
            frame_procurar,
            text="Procurar Aluno (por ID)",
            anchor=tk.W,
            font=("Ivy 10 bold"),
            bg=co1,
            fg=co4,
        ...
        try:
            ...
            self.img_update = ImageTk.PhotoImage(
                Image.open("update.png").resize((20, 20))
            )
            self.img_delete = ImageTk.PhotoImage(
                Image.open("delete.png").resize((20, 20))
            )
        except FileNotFoundError:
    ...
    def _criar_tabela(self):
        ...
        self.arvore_aluno = ttk.Treeview(
            self.frame_tabela,
            selectmode="extended",
            columns=lista_cabeca,
            show="headings",
        )
        vsb = ttk.Scrollbar(
            self.frame_tabela, orient="vertical", command=self.arvore_aluno.yview
        )
        hsb = ttk.Scrollbar(
            self.frame_tabela, orient="horizontal", command=self.arvore_aluno.xview
        )
    ...
    def adicionar(self):
        ...
        if not all([nome, email, telefone, sexo, endereco, curso]):
            mb.showerror(
                "Erro de Validação", "Por favor, preencha todos os campos obrigatórios."
            )
            return

        img_para_salvar = (
            self.caminho_da_imagem if self.caminho_da_imagem else "logo.png"
        )
        ...
        if self.db.registro_de_estudante(dados):
            ...
            mb.showerror(
                "Erro no Banco",
                "Não foi possível registrar o estudante.\nO email informado pode já estar em uso.",
            )
    ...
    def atualizar(self):
        try:
            ...
            mb.showerror(
                "Erro", "ID de busca inválido. Por favor, procure um aluno primeiro."
            )
            return
        ...
        if self.db.atualizar_estudantes(dados_atualizados):
            mb.showinfo(
                "Sucesso", f"Estudante com ID {id_aluno} atualizado com sucesso!"
            )
    ...
    def procurar(self):
        ...
        if not dados:
            mb.showerror(
                "Não Encontrado", f"Nenhum aluno com ID {id_aluno} foi encontrado."
            )
            return
        ...
        try:
            ...
            mb.showwarning(
                "Aviso", f"Imagem não encontrada no caminho: {self.caminho_da_imagem}"
            )
    ...
    def deletar(self):
        try:
            ...
            mb.showerror(
                "Erro",
                "ID de busca inválido. Por favor, procure um aluno antes de deletar.",
            )
            return

        if mb.askyesno(
            "Confirmar",
            f"Tem certeza que deseja deletar o estudante com ID {id_aluno}?",
        ):
